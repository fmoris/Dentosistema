...
$result = mysqli_query($conexion, "SELECT DISTINCT orden.IDORIGEN, origen.INSTITUCION FROM orden, origen WHERE orden.IDODONTOLOGO= '$idOdontologo' AND origen.IDORIGEN = orden.IDORIGEN AND orden.ESTADO = 'terminado' AND orden.PAGADO = 'no pagado' AND orden.anulacion IS NULL ORDER BY orden.IDORIGEN");
		    // el arreglo se popula en este bucle
			while($row = mysqli_fetch_array($result)){
		        
		        // crea un objeto donde se incluyen los datos del registro
			   	$objetoOdontologo = array();
                $objetoOdontologo["nombre"] = $row['INSTITUCION'];
                ...
                $resultadoOrdenado["total"] += $objetoOdontologo["total"];
			   	array_push($resultadoOrdenado["origenes"], $objetoOdontologo);
