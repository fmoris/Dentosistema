 dentosys.controller('editarColados', function($scope, $http, toaster, $window, $routeParams, $location, $anchorScroll, $route) {
    ...
    $scope.obtenerMetales = function(){
     ...
     $scope.obtenerOrden = function() {
         $http.get('sistema/ordenes/colados/obtenerOrden.php', {
             params: {
                 id: $scope.idOrden,
                 clasi: $scope.clasi
     ...
     $scope.obtenerTrabajos();
      ...
      //Funcionalidad Precio Metal
       $scope.precioMetal= function (){
           $scope.orden.totalMetal = parseInt($scope.orden.metal.precio) * parseFloat($scope.orden.pesoMetal);
           $scope.orden.total = parseInt($scope.orden.totalOld) -parseInt($scope.orden.totalMetalOld) + $scope.orden.totalMetal
       }
