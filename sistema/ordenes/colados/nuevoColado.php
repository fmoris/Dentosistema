...
$colados = $request->colados;
$piezas =$request->piezas;
$idMetal = $request->metal->id;
$pesoMetal =$request->pesoMetal;
$totalMetal =$request->totalMetal;
...
$ultimaOrden= mysqli_query($conexion,"SELECT MAX( NUMORDEN ) as idOrden
		...
		if($ultimaOrden){
			...
			if($guardaOrden){			        
				...
				$guardaEspecificos = mysqli_query($conexion,"INSERT INTO colados (NUMORDEN, CLASIFICACION, NUMCORONAS, PIEZAS, IDMETAL, PESOMETAL, TOTALMETAL)
				                                             VALUES ('$idOrden', '$clasi', '$colados', '$piezas', '$idMetal', '$pesoMetal', '$totalMetal')");
