 dentosys.controller('editarProtesisFija', function($scope, $http, toaster, $window, $routeParams, $location, $anchorScroll, $route) {
    ...
    $scope.idOrden = $routeParams.orden;
     ...
     $scope.obtenerOrden = function() {
         $http.get('sistema/ordenes/protesisFija/obtenerOrden.php', {
             params: {
                 id: $scope.idOrden,
                 clasi: $scope.orden.clasi
     ...
     $scope.editarOrden = function(inputData) {
         ...
         $http.post('sistema/ordenes/protesisFija/editarOrden.php', inputData)
             .success(function(outputData) {
                 //$route.reload();
                 $http.post('sistema/ordenes/protesisFija/enviaMailOdontologo.php', inputData)                
                     .success(function(data) {
                         console.log(data);
                         if (data == 1) {
                             toaster.pop('success', "Correo Enviado", 'El correo a sido enviado correctamente', 5000, 'trustedHtml');
                         } else {
                             toaster.pop('error', "Error al Enviar Correo", 'El correo no a sido enviado <br>'+data, null, 'trustedHtml');
                         }
                     })
             })
     }
